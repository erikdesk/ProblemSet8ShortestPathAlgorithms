...
//// MARK: minimumEffortPath - Doesn't work!
func minimumEffortPath(_ heights: [[Int]]) -> Int {
    ...
    let width = heights.count
    let height = heights[0].count
    
    var slopes = [[Int]](repeating: [Int](repeating: Int.max, count: height), count: height)
    slopes[0][0] = 0
    for slope in slopes {
        print(slope)
    }
    
    let directions: [(r: Int, c: Int)] = [(-1, 0), (+1, 0), (0, -1), (0, +1)]
    
    for _ in 0..<width*height {
        for r in 0..<width {
            for c in 0..<height {
                for d in directions {
                    if r + d.r >= 0 && r + d.r < width && c + d.c >= 0 && c + d.c < height {
                        print(r, c, r + d.r, c + d.c)
                        
                        if abs(heights[r + d.r][c + d.c] - heights[r][c]) < slopes[r + d.r][c + d.c] {
                            slopes[r + d.r][c + d.c] = max(slopes[r][c], abs(heights[r + d.r][c + d.c] - heights[r][c]))
                            for slope in slopes {
                                print(slope)
                            }
                        }
    ...
    return slopes[width-1][height-1]
}
